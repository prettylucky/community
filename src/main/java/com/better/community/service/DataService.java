...
/**
 * 数据统计
 * UV：独立访客，一天内多次登录算一次独立访客，使用HyperLogLog来存储
 * 一段时间内的UV，就是把几天的UV加在以前，然后再去重
 * DAU：日活用户，此处我们定义一天内只要登录一次或以上的都算日活用户，使用BitMap存储
 * 一段时间内的AU，就是多几天的DAU做或运算得到的结果
 * @Date 7/23/2022
 */
@Service
public class DataService {
    ...
    public long calculateDAU(Date start, Date end) {
        ...
        // 把日期内的结果做 OR 运算
        String redisKey = RedisKeyUtil.getDAUKey(df.format(start), df.format(end));     //存放OR运算结果的key
