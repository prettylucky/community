package com.better.community.quartz;
...
public class PostScoreRefreshJob implements Job, CommunityConstant {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        BoundSetOperations opeartions = redisTemplate.boundSetOps(redisKey);

        if (opeartions.size() == 0){
            logger.info("[任务取消] 没有需要刷新的帖子");
            return;
        }

        logger.info("[任务开始] 正在刷新帖子分数：" + opeartions.size());

        while (opeartions.size() > 0) {
            Object pop = opeartions.pop();
            System.out.println(pop.toString());
            this.refresh((Integer) pop);
        }

        logger.info("[任务结束] 帖子分数刷新成功！");
    }
